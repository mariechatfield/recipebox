  ...
  return new Promise((resolve, reject) => {
    fs.readFile(filePath, { encoding: 'utf-8' }, (err, rawData) => {
      ...
      try {
        ...
        resolve({ uid: recipeData.uid, filePath: recipeFile, hash: recipeData.hash })
      } catch (e) {
        reject(e)
      }
    })
...
async function updateAuditLog() {
  ...
  const allRecipeData = await Promise.all(allRecipes.map(getAuditDataFromRecipe))

  const auditData = allRecipeData.reduce((memo, recipeData) => ({
    ...memo,
    [recipeData.uid]: recipeData
  }), {})

  fs.writeFileSync(AUDIT_LOG, JSON.stringify(auditData, null, 2), { encoding: 'utf-8' })
}
